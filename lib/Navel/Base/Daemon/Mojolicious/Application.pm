...
sub new {
    ...
    my $authenticated = $routes->under(
        ...
        sub {
            ...
            unless (secure_compare(defined $userinfo ? $userinfo : '', $self->daemon()->{configuration}->{definition}->{webservices}->{credentials}->{login} . ':' . $self->daemon()->{configuration}->{definition}->{webservices}->{credentials}->{password})) {
